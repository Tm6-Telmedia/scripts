...
sqlcmd -S %servidor% -d %base_de_datos% -U %usuario% -P "WdpL0CHymw$tjZaB!3l^A1m9" -Q "TRUNCATE TABLE Especificaciones_productos;"
...
sqlcmd -S %servidor% -d %base_de_datos% -U %usuario% -P "WdpL0CHymw$tjZaB!3l^A1m9" -Q "BULK INSERT [dbo].[Especificaciones_productos] FROM 'C:\inetpub\Telmedia\scripts\Especificaciones.csv' WITH (FIELDTERMINATOR = '|', ROWTERMINATOR = '\n', CODEPAGE = '65001')" >> "C:\inetpub\Telmedia\scripts\log_SQL.txt"
:: Escribir mensaje en el log usando PowerShell
