...
# Validar parámetros
if [[ -z "$sku" || -z "$stock" || -z "$usuario" ]]; then
  echo "Uso: $0 <sku> <stock> <usuario>"
  exit 1
fi
...
if echo "$response" | jq -e '.id' >/dev/null 2>&1; then
  echo "$(date '+%Y-%m-%d %H:%M:%S') - Stock actualizado por el usuario $usuario para el producto SKU:$sku (ID: $id) a $stock unidades." 
  echo "$(date '+%Y-%m-%d %H:%M:%S') - Stock actualizado por el usuario $usuario para el producto SKU:$sku (ID: $id) a $stock unidades." >> status_stock.log
else
