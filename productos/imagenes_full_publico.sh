...
x=$(find *.png *.jpg *.jpeg -mmin -100  )
total=$( find *.png *.jpg *.jpeg  -mmin -100| wc -l)
array=($x)
...
agregar_marca_agua() {
    ...
    tmp=tmp.png
        ...
        medidas=$(echo "$w"x"$h")
            ...
            if [[ $w -lt 300 ]]; then
                echo -e "Agregando marca de agua"
                magick -size 1080x1080  ../../scripts/icon/icon.png -thumbnail $medidas^ -gravity center -extent $medidas +profile "*" $tmp
                magick composite -dissolve 20 -gravity Center "$tmp" "$i" "$i"
            else
                 echo -e "Agregando marca de agua"
                magick -size 1080x1080  ../../scripts/icon/icon.png -thumbnail $medidas^ -gravity center -extent $medidas +profile "*" $tmp
                magick "$i"  ../../scripts/icon/icon1.png -gravity southeast -geometry +10+10 -composite "$i"
                magick composite -dissolve 20 -gravity Center "$tmp" "$i" "$i"
            fi
...
imagenes_cuadradas() {
	if [[ $w -le 600 ]]
		then
			...
			#magick convert "$i" -resize $medidas -background white -gravity center -extent $medidas "$i"
			magick convert "$i" -resize $medidas -background white -gravity center -extent $medidas "$i"
			...
			echo "Haciendo imagen cuadrada" 
			#magick  -size $medidas $i -thumbnail $medidas -gravity center -extent $medidas +profile "*" $i 
			#magick convert  -extent $medidas -background white -flatten -gravity center  $i  $i
			magick convert "$i" -resize $medidas -background white -gravity center -extent $medidas "$i"
...
for i in "${array[@]}"
    ...
    if [[ "$watermark" == *"WEBP"* ]]; then
		...
		imagenes_cuadradas "$w" "$h" "$i" "$medidas_original"
#27/07/2024 Provisionalmente probaremos sin poner marca de agua:
#16/01/2025 Permanentemente probaremos sin poner marca de agua:
		#agregar_marca_agua "$w" "$h" "$i"
...
#NO PARA EVITAR ESPERA
#	read -n1 -r -p "Imágenes actualizadas, pulsa cualquier tecla para salir..." key
	echo "Imágenes actualizadas, pulsando cualquier tecla para salir..." >> /tmp/imagenes_full.log

cd $AQUI
sh imagenes_webp_thumb2.sh

echo Imagenes público >> /tmp/imagenes_full.log
